...
OUT_CSV = "Results/custom_detection_metrics.csv"
...
def main():
    ...
    pred_files = sorted(
        [f for f in os.listdir(PRED_DIR) if f.startswith("pred_") and f.endswith(".json")]
    )
    ...
    for f in pred_files:
        ...
        print(f"Evaluating custom metrics: {run_name}")
        metrics = evaluate_predictions(gt_by_image, path, IOU_THRESH)
        metrics["run"] = run_name
        rows.append(metrics)

    # confidence drop relative to normal
    normal_conf = None
    for row in rows:
        if row["run"] == "normal":
    ...
    print("\n Saved custom metrics to:", OUT_CSV)
    for row in rows:
        print(
            f"{row['run']}: "
