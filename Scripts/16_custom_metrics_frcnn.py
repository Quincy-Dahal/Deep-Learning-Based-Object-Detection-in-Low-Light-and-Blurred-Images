...
PRED_DIR = "Results/preds_coco_frcnn"
OUT_CSV = "Results/frcnn_custom_detection_metrics.csv"
...
def main():
    ...
    pred_files = sorted(
        [f for f in os.listdir(PRED_DIR) if f.startswith("pred_frcnn_") and f.endswith(".json")]
    )

    if not pred_files:
        raise FileNotFoundError(f"No pred_frcnn_*.json files found in {PRED_DIR}")

    rows = []
    for f in pred_files:
        run_name = f.replace("pred_frcnn_", "").replace(".json", "")
        path = os.path.join(PRED_DIR, f)

        print(f"Evaluating Faster R-CNN custom metrics: {run_name}")
    ...
    print("\nSaved Faster R-CNN custom metrics to:", OUT_CSV)
    for row in rows:
        print(
            f"{row['run']}: "
